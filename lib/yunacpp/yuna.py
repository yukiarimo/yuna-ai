...
lib_default = pick_existant_file("yunacpp_default.dll","yuna.so")
lib_failsafe = pick_existant_file("yunacpp_failsafe.dll","yuna_failsafe.so")
lib_openblas = pick_existant_file("yunacpp_openblas.dll","yuna_openblas.so")
lib_noavx2 = pick_existant_file("yunacpp_noavx2.dll","yuna_noavx2.so")
lib_clblast = pick_existant_file("yunacpp_clblast.dll","yuna_clblast.so")
lib_cublas = pick_existant_file("yunacpp_cublas.dll","yuna_cublas.so")
    ...
    if args.noavx2:
        ...
        if not file_exists(lib_openblas) or (os.name=='nt' and not file_exists("libopenblas.dll")):
            ...
            if sys.platform=="darwin":
                print("Mac OSX note: Some people have found Accelerate actually faster than OpenBLAS. To compare, run yunacpp with --noblas instead.")
...
#################################################################
friendlymodelname = "concedo/yunacpp"  # local kobold api apparently needs a hardcoded known HF model name
maxctx = 2048
maxhordectx = 1024
maxhordelen = 256
    ...
    def do_GET(self):
        ...
        if self.path in ["", "/?"] or self.path.startswith(('/?','?')): #it's possible for the root url to have ?params without /
            ...
            if self.embedded_kailite is None:
                response_body = (f"Embedded Kobold Lite is not found.<br>You will have to connect via the main YunaAI client, or <a href='https://lite.YunaAI.net?local=1&port={self.port}'>use this URL</a> to connect.").encode()
            else:
            ...
            response_body = (json.dumps({"result":"yunacpp","version":KcppVersion}).encode())
...
def show_new_gui():
    ...
    root.title("yunacpp v"+KcppVersion)
    ...
    horde_name_var = ctk.StringVar(value="yunacpp")
    ...
    def togglehorde(a,b,c):
        ...
        if usehorde_var.get()==1 and (horde_name_var.get()=="yunacpp" or horde_name_var.get()=="") and model_var.get()!="":
    ...
    def save_config():
        file_type = [("yunacpp Settings", "*.kcpps")]
    ...
    def display_help():
        try:
            ...
            wb.open("https://github.com/LostRuins/yunacpp/wiki")
        except:
...
def show_gui_warning(issue=None):
    ...
    if issue == "No Backend Available":
        messagebox.showerror(title="No Backends Available!", message="yunacpp couldn't locate any backends to use.\n\nTo use the program, please run the 'make' command from the directory.")
        root.destroy()
...
def show_old_gui():
    ...
    if len(sys.argv) == 1:
        ...
        tk.Label(root, text = "yunacpp Easy Launcher",
                font = ("Arial", 12)).grid(row=0,column=0)
        tk.Label(root, text = "(Note: yunacpp only works with GGML model formats!)",
...
def run_horde_worker(args, api_key, worker_name):
    ...
    def make_url_request(url, data, method='POST'):
        try:
            request = None
            headers = {"apikey": api_key,'User-Agent':'yunacpp Embedded Worker v1','Client-Agent':'yunacppEmbedWorker:1'}
            if method=='POST':
    ...
    BRIDGE_AGENT = f"yunacppEmbedWorker:1:https://github.com/LostRuins/yunacpp"
    cluster = "https://horde.YunaAI.net"
...
def main(launch_args,start_server=True):
    ...
    if args.hordeconfig and args.hordeconfig[0]!="":
        ...
        friendlymodelname = "yunacpp/"+args.hordeconfig[0]
...
if __name__ == '__main__':
    print("***\nWelcome to yunacpp - Version " + KcppVersion) # just update version manually
    # print("Python version: " + sys.version)
    parser = argparse.ArgumentParser(description='yunacpp Server')
    ...
    parser.add_argument("--unbantokens", help="Normally, YunaAI prevents the EOS token from being generated. This flag unbans it.", action='store_true')
